...
}`;function M(e){const r=e.replace("#","");return[parseInt(r.slice(0,2),16)/255,parseInt(r.slice(2,4),16)/255,parseInt(r.slice(4,6),16)/255]}function P(e,r,s){const o=e.createShader(r);return o?(e.shaderSource(o,s),e.compileShader(o),e.getShaderParameter(o,e.COMPILE_STATUS)?o:(console.error("[hero3d] shader compile error:",e.getShaderInfoLog(o)),e.deleteShader(o),null)):null}function O(e,r,s){const o=e.createProgram();return o?(e.attachShader(o,r),e.attachShader(o,s),e.linkProgram(o),e.getProgramParameter(o,e.LINK_STATUS)?o:(console.error("[hero3d] program link error:",e.getProgramInfoLog(o)),e.deleteProgram(o),null)):null}function z(e,r,s,o){const t=e.getContext("webgl2",{alpha:!0,antialias:!1,powerPreference:"low-power"});if(!t)return null;const n=P(t,t.VERTEX_SHADER,H),c=P(t,t.FRAGMENT_SHADER,$);if(!n||!c)return null;const i=O(t,n,c);if(!i)return null;t.useProgram(i);const l={resolution:t.getUniformLocation(i,"u_resolution"),time:t.getUniformLocation(i,"u_time"),colors:t.getUniformLocation(i,"u_colors[0]"),accent:t.getUniformLocation(i,"u_accent"),alpha:t.getUniformLocation(i,"u_alpha"),scroll:t.getUniformLocation(i,"u_scroll")},u=[];for(let f=0;f<4;f++){const m=r[f]??"#000000",[w,x,D]=M(m);u.push(w,x,D)}t.uniform3fv(l.colors,new Float32Array(u));const[d,h,a]=M(s);return t.uniform3f(l.accent,d,h,a),t.uniform1f(l.alpha,o),t.disable(t.BLEND),{resize(f,m){const w=Math.max(1,Math.round(f*I)),x=Math.max(1,Math.round(m*I));e.width=w,e.height=x,t.viewport(0,0,w,x),t.uniform2f(l.resolution,w,x)},render(f,m){t.useProgram(i),t.uniform1f(l.time,f),t.uniform1f(l.scroll,m),t.drawArrays(t.TRIANGLES,0,3)}}}function E(e,r,s){const o=e.replace("#","");let t=parseInt(o.slice(0,2),16)/255,n=parseInt(o.slice(2,4),16)/255,c=parseInt(o.slice(4,6),16)/255;const i=Math.max(t,n,c),l=Math.min(t,n,c),u=(i+l)/2,d=i-l;if(d>1e-4){const h=u>.5?d/(2-i-l):d/(i+l);let a=i===t?(n-c)/d+(n<c?6:0):i===n?(c-t)/d+2:(t-n)/d+4;a=a*60+r,a=(a%360+360)%360;const f=(1-Math.abs(2*u-1))*h,m=f*(1-Math.abs(a/60%2-1)),w=u-f/2;a<60?(t=f,n=m,c=0):a<120?(t=m,n=f,c=0):a<180?(t=0,n=f,c=m):a<240?(t=0,n=m,c=f):a<300?(t=m,n=0,c=f):(t=f,n=0,c=m),t=(t+w)*255,n=(n+w)*255,c=(c+w)*255}else t*=255,n*=255,c*=255;return`rgba(${Math.round(t)}, ${Math.round(n)}, ${Math.round(c)}, ${s})`}let L=null,S=null,v=null,p=null,g=null,b=null,y=null,A=!0;const B=window.matchMedia("(prefers-reduced-motion: reduce)").matches;function q(){L!=null&&(cancelAnimationFrame(L),L=null)}function _(){!B&&A&&!document.hidden&&S!=null?L==null&&(L=requestAnimationFrame(R)):q()}function T(){if(!v||!g||!S)return;const e=S.querySelector(".hero-art-img"),r=e?e.offsetWidth:g.width,s=e&&e.offsetHeight?e.offsetHeight:Math.round(r*(g.height/g.width)),o=r,t=s;if(v.style.width=`${r}px`,v.style.height=`${s}px`,v.width=o,v.height=t,b){const n=S.querySelector(".hero-art-canvas3d");n&&(n.style.width=`${r}px`,n.style.height=`${s}px`,b.resize(o,t))}}function C(e){if(!v||!p||!g)return;const r=v.width,s=v.height;p.clearRect(0,0,r,s);const o=p.createLinearGradient(0,0,0,s);o.addColorStop(0,g.background),o.addColorStop(1,g.backgroundBottom),p.fillStyle=o,p.fillRect(0,0,r,s);for(const t of g.fields){const n=t.cx+t.driftAmp*Math.sin(e*t.driftSpeed+t.phase),c=t.cy+t.driftAmp*Math.cos(e*t.driftSpeedY+t.phase*1.3),i=e*t.hueDrift,l=n*r,u=c*s,d=t.rx*r,h=t.ry*s,a=t.colors[1]??t.colors[0],f=t.colors[t.colors.length-1];p.save(),p.translate(l,u),p.scale(1,h/d);const m=p.createRadialGradient(0,0,0,0,0,d);m.addColorStop(0,E(t.colors[0],i,t.opacity)),m.addColorStop(.45,E(a,i,t.opacity*.75)),m.addColorStop(1,E(f,i,0)),p.fillStyle=m,p.beginPath(),p.arc(0,0,d,0,Math.PI*2),p.fill(),p.restore()}}function R(e){const r=e/1e3;C(r),b&&b.render(r,window.scrollY),S&&!S.classList.contains("is-animated")&&S.classList.add("is-animated"),L=requestAnimationFrame(R)}function k(){const e=document.querySelector(".hero-art");if(!e){q(),y&&(y.disconnect(),y=null),S=null,v=null,p=null,g=null,b=null;return}if(e.dataset.bound)return;e.dataset.bound="true";const r=e.getAttribute("data-art-config");if(!r||(S=e,g=JSON.parse(r),v=e.querySelector(".hero-art-canvas"),p=v?v.getContext("2d"):null,!v||!p))return;b=null;const s=e.querySelector(".hero-art-canvas3d");s&&(b=z(s,g.neon,g.accent,.9)),y&&y.disconnect(),A=!0,y=new IntersectionObserver(t=>{A=t[0]?.isIntersecting??!0,_()});const o=e.querySelector(".hero-art-img")??e;y.observe(o),q(),T(),C(0),b&&b.render(0,window.scrollY),S.classList.add("is-animated"),_()}document.addEventListener("astro:page-load",k);document.addEventListener("visibilitychange",_);window.addEventListener("resize",()=>{S&&(T(),C(0),b&&b.render(0,window.scrollY))});k();document.addEventListener("astro:page-load",()=>{const e=document.getElementById("section-indicator"),r=document.getElementById("section-indicator-text");if(!e||!r)return;const s=Array.from(document.querySelectorAll("#main-wrapper h2, #main-wrapper h3"));if(!s.length)return;const o=document.getElementById("main-wrapper"),t=o?o.offsetTop-window.innerHeight*.6:0,n=new Set;function c(){const a=s.find(f=>n.has(f));!a||a.textContent===r.textContent||(r.style.opacity="0",setTimeout(()=>{r.textContent=a.textContent.trim(),r.style.opacity="1"},150))}const i=new IntersectionObserver(a=>{a.forEach(f=>{f.isIntersecting?n.add(f.target):n.delete(f.target)}),c()},{rootMargin:"-20% 0px -70% 0px",threshold:0});s.forEach(a=>i.observe(a));let l=window.scrollY,u=!1;function d(){const a=window.scrollY,f=a>t&&a>l;e.classList.toggle("visible",f),l=a,u=!1}function h(){u||(requestAnimationFrame(d),u=!0)}window.addEventListener("scroll",h,{passive:!0}),h()});
