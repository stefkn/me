...
}`;function I(e){const i=e.replace("#","");return[parseInt(i.slice(0,2),16)/255,parseInt(i.slice(2,4),16)/255,parseInt(i.slice(4,6),16)/255]}function M(e,i,s){const r=e.createShader(i);return r?(e.shaderSource(r,s),e.compileShader(r),e.getShaderParameter(r,e.COMPILE_STATUS)?r:(console.error("[hero3d] shader compile error:",e.getShaderInfoLog(r)),e.deleteShader(r),null)):null}function O(e,i,s){const r=e.createProgram();return r?(e.attachShader(r,i),e.attachShader(r,s),e.linkProgram(r),e.getProgramParameter(r,e.LINK_STATUS)?r:(console.error("[hero3d] program link error:",e.getProgramInfoLog(r)),e.deleteProgram(r),null)):null}function z(e,i,s,r){const t=e.getContext("webgl2",{alpha:!0,antialias:!1,powerPreference:"low-power"});if(!t)return null;const n=M(t,t.VERTEX_SHADER,$),l=M(t,t.FRAGMENT_SHADER,H);if(!n||!l)return null;const d=O(t,n,l);if(!d)return null;t.useProgram(d);const c={resolution:t.getUniformLocation(d,"u_resolution"),time:t.getUniformLocation(d,"u_time"),colors:t.getUniformLocation(d,"u_colors[0]"),accent:t.getUniformLocation(d,"u_accent"),alpha:t.getUniformLocation(d,"u_alpha"),scroll:t.getUniformLocation(d,"u_scroll")},f=[];for(let a=0;a<4;a++){const m=i[a]??"#000000",[g,x,D]=I(m);f.push(g,x,D)}t.uniform3fv(c.colors,new Float32Array(f));const[u,h,o]=I(s);return t.uniform3f(c.accent,u,h,o),t.uniform1f(c.alpha,r),t.disable(t.BLEND),{resize(a,m){const g=Math.max(1,Math.round(a*q)),x=Math.max(1,Math.round(m*q));e.width=g,e.height=x,t.viewport(0,0,g,x),t.uniform2f(c.resolution,g,x)},render(a,m){t.useProgram(d),t.uniform1f(c.time,a),t.uniform1f(c.scroll,m),t.drawArrays(t.TRIANGLES,0,3)}}}function E(e,i,s){const r=e.replace("#","");let t=parseInt(r.slice(0,2),16)/255,n=parseInt(r.slice(2,4),16)/255,l=parseInt(r.slice(4,6),16)/255;const d=Math.max(t,n,l),c=Math.min(t,n,l),f=(d+c)/2,u=d-c;if(u>1e-4){const h=f>.5?u/(2-d-c):u/(d+c);let o=d===t?(n-l)/u+(n<l?6:0):d===n?(l-t)/u+2:(t-n)/u+4;o=o*60+i,o=(o%360+360)%360;const a=(1-Math.abs(2*f-1))*h,m=a*(1-Math.abs(o/60%2-1)),g=f-a/2;o<60?(t=a,n=m,l=0):o<120?(t=m,n=a,l=0):o<180?(t=0,n=a,l=m):o<240?(t=0,n=m,l=a):o<300?(t=m,n=0,l=a):(t=a,n=0,l=m),t=(t+g)*255,n=(n+g)*255,l=(l+g)*255}else t*=255,n*=255,l*=255;return`rgba(${Math.round(t)}, ${Math.round(n)}, ${Math.round(l)}, ${s})`}let y=null,w=null,v=null,p=null,b=null,S=null,L=null,A=!0;const F=window.matchMedia("(prefers-reduced-motion: reduce)").matches;function C(){y!=null&&(cancelAnimationFrame(y),y=null)}function _(){!F&&A&&!document.hidden&&w!=null?y==null&&(y=requestAnimationFrame(R)):C()}function k(){if(!v||!b||!w)return;const e=w.querySelector(".hero-art-img"),i=e?e.offsetWidth:b.width,s=e&&e.offsetHeight?e.offsetHeight:Math.round(i*(b.height/b.width)),r=i,t=s;if(v.style.width=`${i}px`,v.style.height=`${s}px`,v.width=r,v.height=t,S){const n=w.querySelector(".hero-art-canvas3d");n&&(n.style.width=`${i}px`,n.style.height=`${s}px`,S.resize(r,t))}}function P(e){if(!v||!p||!b)return;const i=v.width,s=v.height;p.clearRect(0,0,i,s);const r=p.createLinearGradient(0,0,0,s);r.addColorStop(0,b.background),r.addColorStop(1,b.backgroundBottom),p.fillStyle=r,p.fillRect(0,0,i,s);for(const t of b.fields){const n=t.cx+t.driftAmp*Math.sin(e*t.driftSpeed+t.phase),l=t.cy+t.driftAmp*Math.cos(e*t.driftSpeedY+t.phase*1.3),d=e*t.hueDrift,c=n*i,f=l*s,u=t.rx*i,h=t.ry*s,o=t.colors[1]??t.colors[0],a=t.colors[t.colors.length-1];p.save(),p.translate(c,f),p.scale(1,h/u);const m=p.createRadialGradient(0,0,0,0,0,u);m.addColorStop(0,E(t.colors[0],d,t.opacity)),m.addColorStop(.45,E(o,d,t.opacity*.75)),m.addColorStop(1,E(a,d,0)),p.fillStyle=m,p.beginPath(),p.arc(0,0,u,0,Math.PI*2),p.fill(),p.restore()}}function R(e){const i=e/1e3;P(i),S&&S.render(i,window.scrollY),w&&!w.classList.contains("is-animated")&&w.classList.add("is-animated"),y=requestAnimationFrame(R)}function T(){const e=document.querySelector(".hero-art");if(!e){C(),L&&(L.disconnect(),L=null),w=null,v=null,p=null,b=null,S=null;return}if(e.dataset.bound)return;e.dataset.bound="true";const i=e.getAttribute("data-art-config");if(!i||(w=e,b=JSON.parse(i),v=e.querySelector(".hero-art-canvas"),p=v?v.getContext("2d"):null,!v||!p))return;S=null;const s=e.querySelector(".hero-art-canvas3d");s&&(S=z(s,b.neon,b.accent,.9)),L&&L.disconnect(),A=!0,L=new IntersectionObserver(t=>{A=t[0]?.isIntersecting??!0,_()});const r=e.querySelector(".hero-art-img")??e;L.observe(r),C(),k(),P(0),S&&S.render(0,window.scrollY),w.classList.add("is-animated"),_()}document.addEventListener("astro:page-load",T);document.addEventListener("visibilitychange",_);window.addEventListener("resize",()=>{w&&k()});T();document.addEventListener("astro:page-load",()=>{const e=document.getElementById("section-indicator"),i=document.getElementById("section-indicator-text");if(!e||!i)return;const s=Array.from(document.querySelectorAll("#main-wrapper h2, #main-wrapper h3"));if(!s.length)return;const r=document.getElementById("main-wrapper"),t=r?r.offsetTop-window.innerHeight*.6:0,n=new Set;function l(){const o=s.find(a=>n.has(a));!o||o.textContent===i.textContent||(i.style.opacity="0",setTimeout(()=>{i.textContent=o.textContent.trim(),i.style.opacity="1"},150))}const d=new IntersectionObserver(o=>{o.forEach(a=>{a.isIntersecting?n.add(a.target):n.delete(a.target)}),l()},{rootMargin:"-20% 0px -70% 0px",threshold:0});s.forEach(o=>d.observe(o));let c=window.scrollY,f=!1;function u(){const o=window.scrollY,a=o>t&&o>c;e.classList.toggle("visible",a),c=o,f=!1}function h(){f||(requestAnimationFrame(u),f=!0)}window.addEventListener("scroll",h,{passive:!0}),h()});
